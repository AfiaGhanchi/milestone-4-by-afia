document.getElementById('r-form')?.addEventListener('submit', function (event) {
    ...
    if (proPicInput && nameElement && emailElement && contactElement && experienceElement && educationElement && skillsElement) {
        ...
        ${proPicUrl ? `<img src="${proPicUrl}" alt="Profile Image" class="pro-pic" id="resume-pic">` : ''}
        <p contenteditable="true"><strong>Name:</strong> ${name}</p>
        <p contenteditable="true"><strong>Email Address:</strong> ${email}</p>
        <p contenteditable="true"><strong>Contact:</strong> ${contact}</p>
